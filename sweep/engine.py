...
def _hand_order(card: int):
    """Sort key keeping hands ordered by game value (suit as a stable tiebreak).

    Hands are kept value-sorted so the derived legal-action list — which is
    generated by iterating the hand — reads in ascending card order for the UI
    and any consumer. Order never affects which actions are legal, only their
    sequence.
    """
    return (card_value(card), card)
...
class Game:
    ...
    def _start_round(self):
        ...
        self.hands = [[], []]
        self.hands[fp] = sorted(deck[4:8], key=_hand_order)
        self.hands[1 - fp] = sorted(deck[8:12], key=_hand_order)
        self.deck = deck[12:]
    ...
    def _deal(self, p, n):
        ...
        del self.deck[:n]
        self.hands[p] += dealt
        self.hands[p].sort(key=_hand_order)
